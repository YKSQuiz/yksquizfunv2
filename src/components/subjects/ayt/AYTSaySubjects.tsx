...
const aytSaySubjects = [
  ...
  { id: 'ayt-kimya', label: 'AYT Kimya', icon: '⚗️', color: 'linear-gradient(135deg, #ff6a00 0%, #ee0979 100%)' },
  { id: 'ayt-biyoloji', label: 'AYT Biyoloji', icon: '🧬', color: 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)' },
];

const AYTSaySubjects: React.FC = () => {
  ...
  return (
    <div className="container">
      <div className="header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        />
        <h1 style={{ flex: 1, textAlign: 'center', margin: 0 }}>AYT-Sayısal Ders Seçimi</h1>
        <div style={{ width: 120 }} />
      </div>
      <div className="card" style={{ background: 'linear-gradient(120deg, #e0e7ff 0%, #f8fafc 100%)', boxShadow: '0 8px 40px #43e97b22' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <p style={{ color: '#555', fontSize: 16, marginTop: 8 }}>Sayısal derslerin soruları seni bekliyor!</p>
        </div>
        <div className="category-grid" style={{ margin: '40px 0 20px 0' }}>
          {aytSaySubjects.map((subj, i) => (
            <div
              key={subj.id}
              className="category-card tyt-animated-card"
              onClick={() => {
                if (subj.id === 'ayt-matematik') {
                  navigate('/ayt-matematik-altkonular');
                } else if (subj.id === 'ayt-fizik') {
                  navigate('/ayt-fizik-altkonular');
                } else if (subj.id === 'ayt-kimya') {
                  navigate('/ayt-kimya-altkonular');
                } else if (subj.id === 'ayt-biyoloji') {
                  navigate('/ayt-biyoloji-altkonular');
                } else {
