...
const dinAltKonular = [
  { id: 'bilgi-inanc', label: 'Bilgi ve İnanç', icon: '📖', color: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
  { id: 'islam-ibadet', label: 'İslam ve İbadet', icon: '🕌', color: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' },
  { id: 'ahlak-degerler', label: 'Ahlak ve Değerler', icon: '🤲', color: 'linear-gradient(135deg, #30cfd0 0%, #330867 100%)' },
  { id: 'allah-insan', label: 'Allah İnsan İlişkisi', icon: '✨', color: 'linear-gradient(135deg, #f7971e 0%, #ffd200 100%)' },
  { id: 'hz-muhammed', label: 'Hz. Muhammed (S.A.V.)', icon: '🕋', color: 'linear-gradient(135deg, #f953c6 0%, #b91d73 100%)' },
  { id: 'vahiy-akil-mezhepler', label: 'Vahiy, Akıl ve Mezhepler', icon: '🧠', color: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
  { id: 'din-kultur-medeniyet', label: 'Din, Kültür ve Medeniyet', icon: '🌍', color: 'linear-gradient(135deg, #43cea2 0%, #185a9d 100%)' },
];
...
const TYTDinAltKonular: React.FC = () => {
  ...
  return (
    <div className="container">
      <div className="header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        />
        <h1 style={{ flex: 1, textAlign: 'center', margin: 0 }}>TYT Din - Alt Konular</h1>
        <div style={{ width: 120 }} />
      </div>
        ...
        <div className="category-grid" style={{ margin: '40px 0 20px 0' }}>
          {dinAltKonular.map((subj, i) => (
            <div
              ...
              tabIndex={0}
              onKeyDown={e => { if (e.key === 'Enter') navigate(`/din/${subj.id}`); }}
            >
...
export default TYTDinAltKonular; 