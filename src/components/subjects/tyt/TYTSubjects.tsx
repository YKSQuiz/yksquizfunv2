  ...
  return (
    <div className="container">
      <div className="header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        />
        <h1 style={{ flex: 1, textAlign: 'center', margin: 0 }}>TYT Ders Seçimi</h1>
        <div style={{ width: 120 }} />
      </div>
      <div className="card" style={{ background: 'linear-gradient(120deg, #e0e7ff 0%, #f8fafc 100%)', boxShadow: '0 8px 40px #43e97b22' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <h2 style={{ color: '#6366f1', fontWeight: 800, fontSize: 28, letterSpacing: 1 }}>🎉 Hangi dersi seçmek istersin?</h2>
          <p style={{ color: '#555', fontSize: 16, marginTop: 8 }}>Her dersin kendine özel soruları seni bekliyor!</p>
        </div>
        <div className="category-grid" style={{ margin: '40px 0 20px 0' }}>
          {tytSubjects.map((subj, i) => (
            <div
              key={subj.id}
              className="category-card tyt-animated-card"
              onClick={() => {
                if (subj.id === 'tyt-turkce') {
                  navigate('/tyt-turkce-altkonular');
                } else if (subj.id === 'tyt-tarih') {
                  navigate('/tyt-tarih-altkonular');
                } else if (subj.id === 'tyt-cografya') {
                  navigate('/tyt-cografya-altkonular');
                } else if (subj.id === 'tyt-felsefe') {
                  navigate('/tyt-felsefe-altkonular');
                } else if (subj.id === 'tyt-din') {
                  navigate('/tyt-din-altkonular');
                } else if (subj.id === 'tyt-matematik') {
                  navigate('/tyt-matematik-altkonular');
                } else if (subj.id === 'tyt-fizik') {
                  navigate('/tyt-fizik-altkonular');
                } else if (subj.id === 'tyt-kimya') {
                  navigate('/tyt-kimya-altkonular');
                } else if (subj.id === 'tyt-biyoloji') {
                  navigate('/tyt-biyoloji-altkonular');
                } else {
              ...
              tabIndex={0}
              onKeyDown={e => { if (e.key === 'Enter') console.log(subj.label); }}
            >
